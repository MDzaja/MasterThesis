...
def new_func(prices: pd.Series, labels: pd.Series) -> pd.Series:
    # Ensure prices are filtered to match labels' indices
    prices = prices[prices.index.isin(labels.index)]
    weights = pd.Series(np.NaN, index=labels.index)
    ...
    scaler = MinMaxScaler(feature_range=(0, weights.shape[0]))
    weights = scaler.fit_transform(weights.values.reshape(-1, 1)).flatten()
